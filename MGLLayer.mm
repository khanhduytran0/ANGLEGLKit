...
- (void)setDrawableProperties:(NSDictionary *)properties {
    NSString *colorFormat = properties[kEAGLDrawablePropertyColorFormat];
    if ([colorFormat isEqualToString:kEAGLColorFormatRGBA8]) {
        _drawableColorFormat = MGLDrawableColorFormatRGBA8888;
    } else if ([colorFormat isEqualToString:kEAGLColorFormatRGB565]) {
        _drawableColorFormat = MGLDrawableColorFormatRGB565;
    } else if ([colorFormat isEqualToString:kEAGLColorFormatSRGBA8]) {
        _drawableColorFormat = MGLDrawableColorFormatSRGBA8888;
    }

    NSNumber *retainedBacking = properties[kEAGLDrawablePropertyRetainedBacking];
    if (retainedBacking) {
        _retainedBacking = retainedBacking.boolValue;
    }
